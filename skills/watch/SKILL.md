...
`watch` is not a new search. It is **re-running `protocol.yml`** and asking one question
...
`closes_if`. A gap without a falsifier cannot be watched — send it back to `survey` Phase 4.
...
gap's freshness**, which is what keeps `gap-gate` G1 scorable and the staleness hook silent.
...
is the most valuable message this plugin will ever send them.

Then offer: re-run `gap-gate` to see whether a neighbouring candidate survives, or
`survey` Phase 4 to re-map now that the closing paper is in the corpus.

## Related

`survey` builds and freezes the protocol · `gap-gate` consumes the freshness this
maintains · `/rs:watch` is the command form
