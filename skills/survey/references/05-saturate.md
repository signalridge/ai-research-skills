...
`last_searched_at` current. The survey is now a durable artifact that `watch` can maintain.
...
| **Published after `protocol.created`** | These did not exist when you started. Field growth. | `watch`, not more rounds |
...
`watch` is not a nice-to-have — it is what keeps the artifact true.
...
The protocol is now the subscription. `watch` re-runs this file verbatim, diffs against
...
Then name the exits: `gap-gate` for go/no-go, `related-work` for prose, `decision-brief`
for a build/adopt/skip call, `watch` to keep it alive.
