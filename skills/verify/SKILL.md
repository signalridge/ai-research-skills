...
Mechanical, deterministic, and the cheapest insurance in the plugin. Fabricated citations
are the most common failure of LLM literature work; wrong numbers are the most damaging.

Scope: **integrity, not quality.** Whether the argument is good is `red-team`'s job.
...
`survey` Phase 3 generates `refs.bib` · `related-work` and `decision-brief` produce what
this audits · `red-team` covers argument quality, which this deliberately does not
