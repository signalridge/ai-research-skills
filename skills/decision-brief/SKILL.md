...
**Never search here.** Missing evidence goes back to `survey`.
...
≥64k context appears." Then add it to `gaps.yml` as a `closes_if` so `watch` tests it
...
Unlike `gap-gate`, **do** make a recommendation here — an engineering call is reversible,
bounded, and yours to advise on. But state the uncertainty in the same breath, and make the
evidence auditable underneath it.

## Related

`survey` builds the corpus · `watch` tests the revisit triggers · `verify` checks that
every number in the matrix traces to a table
