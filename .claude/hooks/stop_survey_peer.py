...
CORROBORATION_MIN_INCLUDES = 12  # below this, zero corroboration links says nothing
...
def audit(survey_dir):
    ...
    # 5. Zero corroboration on a corpus read past the abstract — either the field's
    #    results are genuinely uncontested, or extraction stopped at the surface.
    deep = [r for r in includes if r.get("evidence_read") not in (None, "abstract")]
    if len(includes) >= CORROBORATION_MIN_INCLUDES and len(deep) > len(includes) / 2:
        links = sum(
            len((r.get("corroboration") or {}).get(field) or [])
            for r in includes
            for field in ("agrees_with", "conflicts_with")
        )
        if links == 0:
            findings.append(
                f"{slug}: {len(includes)} includes, most read past the abstract, and not "
                f"one agrees_with/conflicts_with link. Either this field's results are "
                f"genuinely uncontested, or extraction stopped at the surface — say which "
                f"you believe, and why."
            )
