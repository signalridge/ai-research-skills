...
Scope: **integrity, not quality.** Whether the argument is good is `rs-red-team`'s job.
...
- **Upstream:** reads `refs.bib` (generated by `rs-survey` Phase 3), `corpus.jsonl`, and
  the drafts produced by `rs-related-work` and `rs-decision-brief`.
- **Writes:** `integrity.md`.
- **Downstream:** findings block delivery until resolved — the mechanical half of the
  audit pair. `rs-red-team` covers argument quality, which this deliberately does not.
