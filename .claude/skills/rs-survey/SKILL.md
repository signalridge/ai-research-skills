...
**You are the only skill allowed to search.** If `rs-gap-gate`, `rs-related-work`, `rs-watch`, or
`rs-decision-brief` needs a paper that isn't in the corpus, it comes back here.
...
.research/survey/<topic-slug>/
  protocol.yml      # the reproducible search contract — `rs-watch` is just re-running this
...
Schemas are installed at `.claude/research-skills/schemas/`. Validate any time:
...
export or `rs-watch` alerts; say which capability is missing and what that costs the survey.
...
in `protocol.yml.budget`. See [SETUP.md](https://github.com/signalridge/research-skills/blob/main/docs/SETUP.md) for the verified failure modes —
...
9. **Every gap carries a `closes_if` falsifier.** Written in advance, re-tested by `rs-watch`.
...
|---|---|
| Decide whether a topic is worth pursuing | `rs-gap-gate` — and it withholds the verdict too |
| Write related-work prose | `rs-related-work` |
| Monitor a topic over time | `rs-watch` |
| Make a build/adopt/skip call | `rs-decision-brief` |
...
- **Writes:** all of `.research/survey/<slug>/` — `protocol.yml`, `corpus.jsonl`,
  `coverage.yml`, `gaps.yml`, `refs.bib`, `notes/`, `log.md`. This is the only skill that
  searches, and the only one that writes these files.
- **Downstream:** `rs-gap-gate`, `rs-related-work` and `rs-decision-brief` consume the
  state; `rs-watch` re-runs the frozen protocol; `rs-red-team` and `rs-verify` audit it.
