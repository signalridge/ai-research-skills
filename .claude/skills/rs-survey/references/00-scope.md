...
For AI work, the venue universe to pick `venues` from, and the default arXiv category set
Phase 1 searches within:

```yaml
# AI venue universe — pick the 3-6 that actually publish your topic
ai_venues: [NeurIPS, ICML, ICLR, ACL, EMNLP, NAACL, COLM, TMLR,
            AAAI, IJCAI, CVPR, ICCV, ECCV, KDD]
# default arXiv categories — narrow to what the topic touches
arxiv_categories: [cs.LG, cs.CL, cs.CV, cs.AI, cs.MA, stat.ML]
```

Do not sweep all fourteen venues — pick the ones that publish your topic, and say why the
rest are out. `TMLR` and workshop tracks catch work the big-three cycle is too slow for.
...
Recurring axis patterns in AI work — steal the ones your question implies, ignore the rest:

| Axis | Typical values | Good for |
|---|---|---|
| method | the approach families the field is split into | almost every survey |
| data | dataset scale, synthetic vs human, domain | training-data and scaling questions |
| evaluation | benchmark, metric, judge (human vs LLM) | "does X actually work" questions |
| efficiency | compute, latency, context length, parameters | systems-flavoured questions |
| supervision | zero/few-shot, fine-tuned, RL, self-improvement | capability questions |

`method` plus one or two of these lands you in the sizing rule below. The axes still come
from the question — this table is a vocabulary, not a checklist.
