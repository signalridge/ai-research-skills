...
`last_searched_at` current. The survey is now a durable artifact that `rs-watch` can maintain.
...
Run another full round — all three recall modes, fresh queries. The new on-topic papers it
...
| **Published after `protocol.created`** | These did not exist when you started. Field growth. | `rs-watch`, not more rounds |
...
`rs-watch` is not a nice-to-have — it is what keeps the artifact true.
...
round rather than repeating all three uniformly.
...
The protocol is now the subscription. `rs-watch` re-runs this file verbatim, diffs against
...
Then name the exits: `rs-gap-gate` for go/no-go, `rs-related-work` for prose, `rs-decision-brief`
for a build/adopt/skip call, `rs-watch` to keep it alive.
