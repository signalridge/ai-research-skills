...
**Never search here.** Missing evidence goes back to `rs-survey`.
...
≥64k context appears." Then add it to `gaps.yml` as a `closes_if` so `rs-watch` tests it
...
Unlike `rs-gap-gate`, **do** make a recommendation here — an engineering call is reversible,
...
- **Downstream:** `rs-watch` tests the revisit triggers the brief registers; `rs-verify`
