...
`rs-watch` is not a new search. It is **re-running `protocol.yml`** and asking one question
that matters more than the rest: *has anything closed one of my gaps?*

## Two modes

```
/rs-watch arm      # register the standing subscription, once
/rs-watch check    # run a digest now  (also: "what's new on X")
```

## Preconditions

`protocol.yml` at `phase: 5` with a `last_searched_at`, and `gaps.yml` whose entries carry
`closes_if`. A gap without a falsifier cannot be watched — send it back to `rs-survey` Phase 4.
...
gap's freshness**, which is what keeps `rs-gap-gate` G1 scorable and the staleness hook silent.
...
Then offer: re-run `rs-gap-gate` to see whether a neighbouring candidate survives, or
`rs-survey` Phase 4 to re-map now that the closing paper is in the corpus.

## Handoffs

- **Upstream:** re-runs `protocol.yml` verbatim — the contract `rs-survey` froze at
  `phase: 5`.
- **Writes:** appends to `corpus.jsonl`, writes `digest.md`, updates `last_searched_at`.
- **Downstream:** keeps `rs-gap-gate` G1 scorable and the staleness hook silent by
  re-testing every gap's `closes_if` against each new paper.
- `/rs-watch` is the command form.
