...
disallowed-tools:
  ...
  # body is advisory; this makes it structural. A missing paper goes back to `rs-survey`.
...
If either is missing, or `protocol.yml` has `phase < 4`, stop and run `rs-survey` first.
...
field means G1 is being scored against stale evidence; say so, and offer `rs-watch` first.
...
| The strongest baseline you must reproduce or beat was published >12 months ago, in a field with `saturation.baseline_growth` >2×/yr | `corpus.jsonl` + `protocol.yml` | The real baseline is unpublished work. G2 and G3 would be scoring a strawman. |
...
`rs-survey` Phase 4 Step B to discriminate the cell, re-run Phase 1 Mode D, run `/rs-watch
...
| `abandoned`, non-null `revivable_by` | 4–5 | Read as the "newly possible" shape: the failure is attributed to a specific tool and the corpus documents its successor. Name the successor in the dossier — it is the whole thesis. |
...
| `abandoned` | 1 | People tried and it did not hold up, and no successor lifts the blocker. Read the failure before considering repeating it. |
...
`rs-survey` Phase 4 Step C rather than guessing; guessing here is how the best gap in the grid
...
never discriminated. Not scored. To clear: run `rs-survey` Phase 4 Step B on this cell.
**Salvage path:** <the narrower slice worth re-searching — a no-go without one is a dead
end that learned nothing>
...
**Opening type.** Name each candidate's opening in plain words: a **method-limitation**
opening (an existing approach cannot do X) or an **unoccupied-application** opening (a
capability has not been applied to Y). The type decides what kills it — a limitation claim
dies to one counterexample, an unoccupied application dies to one prior application — so
the kill test below is shaped by it. An unoccupied-application opening is not
automatically incremental: say explicitly whether the new domain amounts to a new
*capability* (applying the method to Y forces something it could not do before) or merely
a new venue for an existing one.
...
**Salvage path.** Every `no-go` candidate gets one line: the narrower slice worth a fresh
search — the sub-setting, the weaker claim, the adjacent cell. A no-go is a verdict on
this framing, not on the territory, and a rejection that points nowhere wastes the
assessment that produced it. The slice is a re-search target for `rs-survey`, not a
candidate in this dossier.
...
## Integrity gate — run silently before delivering

Each check is tagged by who can verify it. **[inspection]** you can verify from the
dossier itself. **[attestation]** you state the check was run; the user confirms.

1. **[inspection]** Every gate score cites specific state — file, field, value. No score
   is "gut feeling", and a score without a citation is a defect.
2. **[inspection]** The composite is consistent with the gates: `conditional` or better
   requires Gate 0 clear and no gate at 1–2. A composite that disagrees with its own
   gates is a bug, not a judgement call.
3. **[inspection]** Every `conditional` candidate carries a kill test, and the kill test
   is cheaper than the work it guards.
4. **[inspection]** Absence claims match `gaps.yml` confidence — nothing in the dossier
   asserts more than the evidence of absence supports.
5. **[attestation]** G3 used the user's stated resources (weekly hours, compute, data
   access). Where you assumed, the assumption is written down.
6. **[attestation]** The shelf-life estimate is reasoned from the field's measured growth,
   and the user is told to sanity-check it against their own sense of the subfield.

If an [inspection] check fails, fix the dossier and say nothing about the gate. A failure
surfaces as a correction inside the affected section — the delivered dossier stays free
of internal checking rituals.

## Handoffs

- **Upstream:** reads `coverage.yml`, `gaps.yml` and the corpus written by `rs-survey`;
...
- **Downstream:** the researcher's own go/no-go call — this skill withholds it.
  `rs-red-team` checkpoint B should pass before the dossier is trusted, and `rs-watch`
  keeps G1's freshness current.
- Never searches. Missing evidence goes back to `rs-survey`.
