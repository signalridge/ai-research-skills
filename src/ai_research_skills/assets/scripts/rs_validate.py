        ...
        for num in r.get("numbers") or []:
            if not num.get("looked_at"):
                warn(
                    ...
                    "`ars-verify` will block it; read the table or drop the number",
                )
    ...
    for cell in cov.get("cells") or []:
        ...
        if state in ("unexplored", "avoided") and not cell.get("trend_evidence"):
            err(
                ...
                # Name the state the cell actually carries. `avoided` and `unexplored`
                # score at opposite ends of G2, so telling an author their `avoided` cell
                # is marked `unexplored` points the fix at the wrong field.
    ...
    for g in gaps.get("gaps") or []:
        ...
        if not g.get("closes_if"):
            err(
                ...
                "without it `ars-watch` cannot re-test the gap and it closes silently",
            )
