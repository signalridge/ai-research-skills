...
description: >
  ...
  thematically by taxonomy axis, every claim carrying a corpus key, and every new BibTeX
  entry carrying strict rs-provenance. Use when the user asks to write related work, a literature review
...
`evidence_read`, `coverage.yml` populated, and `refs.bib` present with strict per-entry
`rs-provenance` attestations reconciled to corpus key/id. This is a read-only projection of
source state.
...
**Prefer a within-sentence comparison whenever the corpus supplies two records.** A
single-record paragraph should be treated as a summary and either joined to a comparison or
labeled as limited evidence. Before handoff, inspect paragraph-level keys so synthesis is not
mistaken for a sequence of isolated abstracts.
...
When an extracted record does not state a table value, write **not reported** rather than
leaving a blank that could be read as zero, unknown or omitted by accident. Do not infer a
value to fill the cell; cite the record and let `ars-verify` trace only sourced numbers.

## Citations

`refs.bib` should come from the authorised citation export, with a strict per-entry
`rs-provenance: key=… id=… tool=… date=…` attestation. This is a defensive tripwire, not
cryptographic proof; attestations can be forged and `ars-verify` must resolve identifiers.
Do not treat this projection as permission to discover or append unscreened papers.
...
- **Upstream:** reads `corpus.jsonl`, `coverage.yml` and the attested `refs.bib` written by
  `ars-survey`.
- **Writes:** `related_work.md` only, a read-only projection of the survey ledgers. It never
  edits corpus/protocol/coverage/gaps or performs discovery; missing evidence goes back to
  `ars-survey`.
