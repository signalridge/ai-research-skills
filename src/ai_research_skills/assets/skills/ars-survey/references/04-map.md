...
Count includes by `found_via` mode. A record found by two modes counts under both, so these
do not sum to the include count — that overlap is the healthy case, not double-counting to
be corrected.

```yaml
recall_diagnostic:
  includes_by_mode: {keyword: 44, citation_chain: 31, venue_author: 12, contrarian: 6}
  unsure_by_mode: {venue_author: 4}
...
Count **includes** only. Folding an `unsure` candidate into `includes_by_mode` credits a
mode with work that is still unadjudicated, which is the one number this diagnostic exists
to keep honest. Record those under the optional `unsure_by_mode` instead: a mode whose only
hits are unresolved did contribute recall, and that is a different situation from a mode
...
them in `trend_evidence` with keys — `ars-gap-gate` G2 reads exactly this.
...
gap statement names that successor; `ars-gap-gate` reads the cell as the "newly possible"
...
This is what `ars-watch` re-tests against every new paper, forever. It must be decidable from a
