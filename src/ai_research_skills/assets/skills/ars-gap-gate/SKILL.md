...
If either is missing, or `protocol.yml` has `phase < 4`, stop and run `ars-survey` first.
...
`ars-survey` Phase 4 Step B to discriminate the cell, re-run Phase 1 Mode D, run `/ars-watch
...
`ars-survey` Phase 4 Step C rather than guessing; guessing here is how the best gap in the grid
...
never discriminated. Not scored. To clear: run `ars-survey` Phase 4 Step B on this cell.
...
assessment that produced it. The slice is a re-search target for `ars-survey`, not a
